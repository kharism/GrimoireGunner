...
func NewSwordwomen(ecs *ecs.ECS, col, row int) {
	entity := archetype.NewNPC(ecs.World, assets.Swordswomen)
	entry := ecs.World.Entry(*entity)
	entry.AddComponent(component.EnemyTag)
	component.Health.Set(entry, &component.HealthData{HP: 1000, Name: "Swordwoman"})
...
func (a *moveDagger) Execute(ecs *ecs.ECS) {
	if a.entry.HasComponent(component.Speed) {
		aa := component.Speed.Get(a.entry)
		aa.Vx = -15
		component.Speed.Set(a.entry, aa)
	}

}

func (a *moveDagger) GetTime() time.Time {
	return a.Time
}
